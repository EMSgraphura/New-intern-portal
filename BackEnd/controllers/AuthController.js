...
export const registerUser = async (req, res) => {
  try {
    ...
    } else if (role === "HR Manager" && secretKey !== process.env.HR_MANAGER_SECRET_KEY) {
      return res.status(403).json({ message: "Invalid HR Manager secret key." });
    }
...
export const loginUser = async (req, res) => {
  try {
    const { email, password, role, loginMeta } = req.body;
    ...
    try {
      ...
      console.error("Failed to save admin/hr login log:", logError);
    }
...
const getOtpStoreKey = (email, role) =>
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Save OTP in memory
    const otpKey = getOtpStoreKey(email, role);
