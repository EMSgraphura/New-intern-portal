...
export const getInchargeResignations = async (req, res) => {
  try {
    const inchargeDepartments = req.user.departments;

    const resignations = await Resignation.find()
      .populate({
        path: "intern",
        match: {
          domain: { $in: inchargeDepartments },
        },
        select:
          "fullName email mobile dob gender duration joiningDate domain",
      })
      .sort({ createdAt: -1 });
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to fetch resignations",
    });
  }
};
...
export const getResignations = async (req, res) => {
  try {
    const resignations = await Resignation.find()
      .populate(
        "intern",
        "fullName email mobile dob gender duration joiningDate"
      )
      .sort({ createdAt: -1 });
...
export const updateStatus = async (req, res) => {
  try {
    ...
    // Check if trying to set to Active/Inactive/Completed without required
...
export const approveTerminationAppeal = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    intern.terminationAppeal = false;
    await intern.save();

    try {
      const subject = `Internship Termination Notice – Graphura India Private Limited`;
      const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
            .container { max-width: 600px; margin: 0 auto; padding: 20px; }
            .header { background: linear-gradient(135deg, #e53e3e 0%, #c53030 100%); padding: 30px; text-align: center; color: white; border-radius: 10px 10px 0 0; }
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
            .status-terminated { background: #fed7d7; color: #9b2c2c; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold; }
            .footer { text-align: center; margin-top: 20px; color: #666; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1>Internship Termination</h1>
              <p>Graphura Internship Program</p>
            </div>
            <div class="content">
              <div class="status-terminated">
                ❌ Your internship has been terminated
              </div>
              <p>Dear ${intern.fullName},</p>
              <p>We regret to inform you that your internship at Graphura India Private Limited has been terminated with immediate effect due to unsatisfactory performance or repeated warnings.</p>
              <p>If you have any questions or require clarification, please reach out to the Admin/HR office.</p>
            </div>
            <div class="footer">
              <p>This is an automated notification from Graphura Internship Program.</p>
              <p>© ${new Date().getFullYear()} Graphura India Private Limited. All rights reserved.</p>
      ...
      console.error("Error sending termination email:", emailErr);
    }

    res.json({
      success: true,
      message: "Intern terminated successfully and appeal approved.",
      intern
    });
  } catch (error) {
    console.error("Error approving termination appeal:", error);
    res.status(500).json({
      success: false,
...
export const rejectTerminationAppeal = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    intern.terminationAppeal = false;
    intern.terminationAppealReason = "";
    await intern.save();

    res.json({
      success: true,
      message: "Termination appeal rejected. Intern remains active.",
      intern
    });
  } catch (error) {
    console.error("Error rejecting termination appeal:", error);
    res.status(500).json({
      success: false,
      message: "Internal server error"
    });
...
export const overrideAttendance = async (req, res) => {
  try {
    ...
    // Prepare start and end of the chosen date
    ...
    if (attendance) {
      ...
      attendance.remarks = remarks || "Overridden by Admin";
      ...
      attendance = new Attendance({
        ...
        status,
        remarks: remarks || "Overridden by Admin"
      });
      await attendance.save();
    }

    // Recalculate Intern stats
    const allAtt = await Attendance.find({ intern: internId });
    const presentCount = allAtt.filter(a => a.status === "Present").length;
    const leaveCount = allAtt.filter(a => a.status === "Leave").length;
    const totalMeetings = allAtt.length;

    intern.meetingsAttended = presentCount;
    intern.leavesTaken = leaveCount;
    intern.totalMeetings = totalMeetings;
    await intern.save();

    res.status(200).json({
      success: true,
      message: `Attendance overridden to ${status} for ${intern.fullName}`,
      attendance
    });

  } catch (error) {
    console.error("Error in overrideAttendance:", error);
...
export const checkAttendance = async (req, res) => {
  try {
    const { internId, date } = req.query;

    if (!internId || !date) {
      return res.status(400).json({ success: false, message: "Missing internId or date parameter" });
    }
