      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {/* Interview Invite Count & History Card */}
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                📧 Interview Outreach History
              </h3>
              
              {/* Summary Count */}
              <div className={`border-2 rounded-xl p-4 text-center mb-4 ${intern.interviewCount > 0 ? "bg-purple-50 border-purple-200" : "bg-gray-50 border-gray-200"}`}>
                <div className="text-3xl mb-1">✉️</div>
                <div className="text-xl font-bold">
                  {intern.interviewCount || 0} Invite{intern.interviewCount !== 1 ? 's' : ''} Sent
                </div>
              </div>

              {/* History List */}
              {intern.interviewHistory && intern.interviewHistory.length > 0 ? (
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {intern.interviewHistory.map((invite, index) => (
                    <div key={invite._id} className="p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-purple-200 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[10px] font-bold text-purple-600 uppercase tracking-wider bg-purple-100 px-2 py-0.5 rounded">
                          Batch #{intern.interviewHistory.length - index}
                        </span>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                        <div className="flex flex-col">
                          <span className="text-gray-400 font-bold uppercase text-[9px]">Date</span>
                          <span className="font-bold text-gray-700">{invite.interviewDate}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-gray-400 font-bold uppercase text-[9px]">Time</span>
                          <span className="font-bold text-gray-700">{invite.interviewTime}</span>
                        </div>
                      </div>
                      <div className="text-[10px] text-gray-500 border-t border-gray-100 pt-2">
                        Sent by: <span className="font-bold text-indigo-600">{invite.sender?.fullName}</span> 
                        <span className="ml-1 text-[8px] bg-gray-200 px-1 rounded">{invite.sender?.role}</span>
                      </div>
                    </div>
                  ))}
                </div>
