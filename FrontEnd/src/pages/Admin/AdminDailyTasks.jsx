...
export default function AdminDailyTasks() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !window.confirm(
        "This will permanently delete ALL daily task history of selected interns. Continue?"
      )
    ) return;
    ...
    try {
      ...
      setTasks(prev =>
        prev.filter(t => !selectedIds.includes(t.internId))
      );
      setSelectedIds([]);
  ...
  /* ================= COUNTS ================= */
  const totalInterns = new Set(tasks.map(t => t.internId)).size;
